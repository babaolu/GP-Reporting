    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-bold font-display text-primary-text mt-1">
              {submittedUnits} of {totalUnits} active departments submitted
            </div>
          </div>
          <span className="text-3xl font-bold font-mono text-primary">{submissionRate}%</span>
        </div>
      ...
      {criticalAlerts.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-3xl p-6 shadow-sm space-y-4">
          <div className="flex items-center space-x-2 text-red-800 font-bold font-display text-lg">
          ...
          {unitsData.map((unit) => {
            ...
            return (
              <div
                ...
                className={`bg-white p-5 rounded-3xl border ${config.border} shadow-xxs transition-transform-shadow cursor-pointer flex flex-col justify-between h-48`}
              >
                <div>
                  <div className="flex items-start justify-between">
                    <h4 className="text-lg font-bold font-display text-primary-text max-w-[70%] truncate">
                      {unit.name}
                    </h4>
                    
                    <span className={`text-xxs font-bold px-2 py-0.5 rounded-full border flex items-center space-x-1 ${config.badgeClass}`}>
                ...
                <div className="border-t border-gray-50 pt-3 flex items-center justify-between text-xxs text-gray-400">
                  {unit.report ? (
                    <span className="flex items-center">
                      <FileText className="h-3.5 w-3.5 mr-1 text-primary" />
                      Submitted: {new Date(unit.report.submitted_at).toLocaleDateString()}
                    </span>
                  ) : (
                    <span>No reports uploaded.</span>
                  )}
                  <span className="text-primary font-bold hover:underline flex items-center">
