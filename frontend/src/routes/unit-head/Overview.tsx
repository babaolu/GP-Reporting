    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0">
        <div className="flex items-center space-x-4">
          <div className="h-16 w-16 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-center">
            <User className="h-8 w-8 text-primary" />
          </div>
          <div>
            <h1 className="text-3xl md:text-4xl font-bold font-display text-primary-text leading-tight">
        ...
        <div className="flex flex-col text-left md:text-right">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Reporting Period</span>
          <span className="text-lg font-bold text-primary-text font-display mt-0.5">
      ...
      {user && !user.telegram_linked && (
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex items-start space-x-3 shadow-sm transition-transform-shadow">
          ...
          <button
            ...
            className="text-xs font-bold text-amber-900 hover:text-amber-950 underline px-3 py-1.5 rounded-lg hover:bg-amber-100/50 transition-colors shrink-0 cursor-pointer"
          >
            Link Bot Account
          </button>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between space-y-6">
          <div>
            ...
            {isLoadingReport ? (
              ...
              <div className="flex items-center space-x-4">
                <div className="bg-green-100 text-green-custom p-3 rounded-2xl">
                  <CheckCircle2 className="h-8 w-8" />
                </div>
                <div>
                  <div className="text-lg font-bold text-green-700 font-display">Submitted Successfully</div>
              ...
              <div className="flex items-center space-x-4">
                <div className="bg-red-100 text-danger-custom p-3 rounded-2xl">
                  <AlertTriangle className="h-8 w-8" />
                </div>
                <div>
                  <div className="text-lg font-bold text-red-700 font-display">Overdue Submission</div>
                  <p className="text-xs text-gray-500 mt-0.5">
              ...
              <div className="flex items-center space-x-4">
                <div className="bg-indigo-50 text-primary p-3 rounded-2xl">
                  <Clock className="h-8 w-8" />
                </div>
                <div>
                  <div className="text-lg font-bold text-primary-text font-display">Pending Submission</div>
                  <p className="text-xs text-gray-500 mt-0.5">
          ...
          <div className="border-t border-gray-50 pt-4">
            <button
              ...
              className="w-full flex items-center justify-between bg-primary text-white py-3 px-5 rounded-2xl hover:bg-indigo-800 transition-colors font-semibold shadow-sm cursor-pointer text-sm"
            >
          ...
          <div>
            ...
            {deadlineLoading ? (
              ...
              <div className="space-y-2">
                <div className="text-3xl font-bold font-display text-primary-text">
                  {daysRemaining} Day(s) Left
                </div>
                <div className="text-sm font-semibold text-gray-600">
                  Due on: <span className="text-accent">{deadline?.deadline_date ? new Date(deadline.deadline_date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'}</span>
                </div>
